...
class CapitoPositiveDataset(DatasetAbstract):
    ...
    def initialize_data_set_parameters(self):
        ...
        self.corpus_attribute_name = {  CapitoPositiveDataset.SOURCE+CapitoPositiveDataset.SENTENCE_TOKEN_NAME:"src_text",   
                                        CapitoPositiveDataset.TARGET+CapitoPositiveDataset.SENTENCE_TOKEN_NAME:"trg_text",  
                                        CapitoPositiveDataset.SCORE_TOKEN_NAME:"aligment"
        }   
        self.data_set_type_name = {CapitoPositiveDataset.DATA_SET_TYPE_TRAIN:"train", 
                                   CapitoPositiveDataset.DATA_SET_TYPE_VALIDATION:"validation", 
                                   CapitoPositiveDataset.DATA_SET_TYPE_TEST:"test"  
        }
        

        self.path_to_data = self.params["capito_positive"]
                
        self.type_to_file = {self.data_set_type_name[CapitoPositiveDataset.DATA_SET_TYPE_TRAIN]:"train.tsv",   
                             self.data_set_type_name[CapitoPositiveDataset.DATA_SET_TYPE_TEST]:"test.tsv",  
                             self.data_set_type_name[CapitoPositiveDataset.DATA_SET_TYPE_VALIDATION]:"dev.tsv"}      
    ...
    def _load_data_set(self, data_set_type, limit):
        ...
        if limit:
            ...
            dataset = dataset.filter(
                lambda x: True if x['aligment'] == 'partialAligned' or x['aligment'] =='aligned' else False
            )
            dataset = dataset.map(lambda x: {"aligment": 1} , batched=False)
            return dataset
